#include "bsp_MB_host.h"
#include"HeadType.h"
/* 私有类型定义 --------------------------------------------------------------*/
/* 私有宏定义 ----------------------------------------------------------------*/
/* 私有变量 ------------------------------------------------------------------*/

// CRC 高位字节值表
static const uint8_t auchCRCHi[] = {
    
} ;
// CRC 低位字节值表
static const uint8_t auchCRCLo[] = {
	
};
/* 扩展变量 ------------------------------------------------------------------*/
/* 私有函数原形 --------------------------------------------------------------*/
/* 函数体 --------------------------------------------------------------------*/

/** 
  * 函数功能: Modbus CRC16 校验计算函数
  * 输入参数: pushMsg:待计算的数据首地址,usDataLen:数据长度
  * 返 回 值: CRC16 计算结果
  * 说    明: 计算结果是高位在前,需要转换才能发送
  */
uint16_t MB_CRC16(uint8_t *pushMsg,uint8_t usDataLen)
{
  uint8_t uchCRCHi = 0xFF;
  uint8_t uchCRCLo = 0xFF;
  uint16_t uIndex;
  while(usDataLen--)
  {
    uIndex = uchCRCLo ^ *pushMsg++;
    uchCRCLo = uchCRCHi^auchCRCHi[uIndex];
    uchCRCHi = auchCRCLo[uIndex];
  }
  return (uchCRCHi<<8|uchCRCLo);
}
void UART_Transmit_IT(Usart_Type *usart)
{
		USART_SendData(usart->huart,usart->txbuf[usart->tx_index++]);
}
/** 
  * 函数功能: 读N个线圈状态(CoilStatue)
  * 输入参数: _addr:从站地址,_reg:寄存器地址,_num:待读取的数量
  * 返 回 值: 无
  * 说    明: 填充数据发送缓存区,然后发送
  */
void MB_ReadCoil_01H(Usart_Type *usart,uint8_t _addr, uint16_t _reg, uint16_t _num)
{
  uint16_t crc = 0;
	
//	while(usart->tx_count!=0);
	wait_tx_count_reset(usart->tx_count);
	usart->tx_count = 0;	
	usart->txbuf[usart->tx_count++] = _addr;		    /* 从站地址 */
	usart->txbuf[usart->tx_count++] = 0x01;		      /* 功能码 */	
	usart->txbuf[usart->tx_count++] = _reg >> 8;	  /* 寄存器地址 高字节 */
	usart->txbuf[usart->tx_count++] = _reg;		      /* 寄存器地址 低字节 */
	usart->txbuf[usart->tx_count++] = _num >> 8;	  /* 线圈(bit)个数 高字节 */
	usart->txbuf[usart->tx_count++] = _num;		      /* 线圈(bit)个数 低字节 */

	crc = MB_CRC16(usart->txbuf,usart->tx_count);
  usart->txbuf[usart->tx_count++] = crc;	          /* crc 低字节 */
	usart->txbuf[usart->tx_count++] = crc>>8;		      /* crc 高字节 */
	MODBUS_RE485_SEND();
	usart->tx_index = 0;	
	UART_Transmit_IT(usart);
	usart->rx_aframe = 0;	//清空和主机的通讯，避免通讯错误
	usart->rx_count = 0;	
}
/** 
  * 函数功能: 写单个线圈状态(CoilStatue)
  * 输入参数: _addr:从站地址,_reg:寄存器地址,_sta:待写入的线圈状态(0,1)
  * 返 回 值: 无
  * 说    明: 填充数据发送缓存区,然后发送
  */
void MB_WriteCoil_05H(Usart_Type *usart,uint8_t _addr, uint16_t _reg, uint16_t _sta)
{
  uint16_t crc = 0;
	
//	while(usart->tx_count!=0);
	wait_tx_count_reset(usart->tx_count);
	usart->tx_count = 0;	
	usart->txbuf[usart->tx_count++] = _addr;		    /* 从站地址 */
	usart->txbuf[usart->tx_count++] = 0x05;		      /* 功能码 */	
	usart->txbuf[usart->tx_count++] = _reg >> 8;	  /* 寄存器地址 高字节 */
	usart->txbuf[usart->tx_count++] = _reg;		      /* 寄存器地址 低字节 */
	usart->txbuf[usart->tx_count++] = _sta >> 8;	  /* 线圈(bit)个数 高字节 */
	usart->txbuf[usart->tx_count++] = _sta;		      /* 线圈(bit)个数 低字节 */

	crc = MB_CRC16(usart->txbuf,usart->tx_count);
  usart->txbuf[usart->tx_count++] = crc;	          /* crc 低字节 */
	usart->txbuf[usart->tx_count++] = crc>>8;		      /* crc 高字节 */
	MODBUS_RE485_SEND();
	usart->tx_index = 0;	
	UART_Transmit_IT(usart);
	usart->rx_aframe = 0;	//清空和主机的通讯，避免通讯错误
	usart->rx_count = 0;		
}

/** 
  * 函数功能: 读输入状态状态(InputStatue)
  * 输入参数: _addr:从站地址,_reg:寄存器地址,_num:待读取的输入数量
  * 返 回 值: 无
  * 说    明: 填充数据发送缓存区,然后发送
  */
void MB_ReadInput_02H(Usart_Type *usart,uint8_t _addr, uint16_t _reg, uint16_t _num)
{
  uint16_t crc = 0;
	
//	while(usart->tx_count!=0);
	wait_tx_count_reset(usart->tx_count);
	usart->tx_count = 0;	
	usart->txbuf[usart->tx_count++] = _addr;		    /* 从站地址 */
	usart->txbuf[usart->tx_count++] = 0x02;		      /* 功能码 */	
	usart->txbuf[usart->tx_count++] = _reg >> 8;	  /* 寄存器地址 高字节 */
	usart->txbuf[usart->tx_count++] = _reg;		      /* 寄存器地址 低字节 */
	usart->txbuf[usart->tx_count++] = _num >> 8;	  /* 线圈(bit)个数 高字节 */
	usart->txbuf[usart->tx_count++] = _num;		      /* 线圈(bit)个数 低字节 */

	crc = MB_CRC16(usart->txbuf,usart->tx_count);
  usart->txbuf[usart->tx_count++] = crc;	          /* crc 低字节 */
	usart->txbuf[usart->tx_count++] = crc>>8;		      /* crc 高字节 */
	MODBUS_RE485_SEND();
	usart->tx_index = 0;	
	UART_Transmit_IT(usart);
	usart->rx_aframe = 0;	//清空和主机的通讯，避免通讯错误
	usart->rx_count = 0;		
}
/** 
  * 函数功能: 读保持寄存器(HoldingRegister)
  * 输入参数: _addr:从站地址,_reg:寄存器地址,_num:待读取的寄存器数量
  * 返 回 值: 无
  * 说    明: 填充数据发送缓存区,然后发送
  */
void MB_ReadHoldingReg_03H(Usart_Type *usart,uint8_t _addr, uint16_t _reg, uint16_t _num)
{
  uint16_t crc = 0;

//	while(usart->tx_count!=0);
	wait_tx_count_reset(usart->tx_count);
	usart->tx_count = 0;	
	usart->txbuf[usart->tx_count++] = _addr;		    /* 从站地址 */
	usart->txbuf[usart->tx_count++] = 0x03;		      /* 功能码 */	
	usart->txbuf[usart->tx_count++] = _reg >> 8;	  /* 寄存器地址 高字节 */
	usart->txbuf[usart->tx_count++] = _reg;		      /* 寄存器地址 低字节 */
	usart->txbuf[usart->tx_count++] = _num >> 8;	  /* 线圈(bit)个数 高字节 */
	usart->txbuf[usart->tx_count++] = _num;		      /* 线圈(bit)个数 低字节 */

	crc = MB_CRC16(usart->txbuf,usart->tx_count);
  usart->txbuf[usart->tx_count++] = crc;	          /* crc 低字节 */
	usart->txbuf[usart->tx_count++] = crc>>8;		      /* crc 高字节 */
	MODBUS_RE485_SEND();
	usart->tx_index = 0;	
	UART_Transmit_IT(usart);
	usart->rx_aframe = 0;	//清空和主机的通讯，避免通讯错误
	usart->rx_count = 0;		
}
/** 
  * 函数功能: 写单个保持寄存器(HoldingRegister)
  * 输入参数: _addr:从站地址,_reg:寄存器地址,_data:待写入的寄存器数据
  * 返 回 值: 无
  * 说    明: 填充数据发送缓存区,然后发送
  */
void MB_WriteHoldingReg_06H(Usart_Type *usart,uint8_t _addr, uint16_t _reg, uint16_t _data)
{
  uint16_t crc = 0;
	
//	while(usart->tx_count!=0);
	wait_tx_count_reset(usart->tx_count);
	usart->tx_count = 0;	
	usart->txbuf[usart->tx_count++] = _addr;		    /* 从站地址 */
	usart->txbuf[usart->tx_count++] = 0x06;		      /* 功能码 */	
	usart->txbuf[usart->tx_count++] = _reg >> 8;	  /* 寄存器地址 高字节 */
	usart->txbuf[usart->tx_count++] = _reg;		      /* 寄存器地址 低字节 */
	usart->txbuf[usart->tx_count++] = _data >> 8;	  /* 线圈(bit)个数 高字节 */
	usart->txbuf[usart->tx_count++] = _data;		      /* 线圈(bit)个数 低字节 */

	crc = MB_CRC16(usart->txbuf,usart->tx_count);
  usart->txbuf[usart->tx_count++] = crc;	          /* crc 低字节 */
	usart->txbuf[usart->tx_count++] = crc>>8;		      /* crc 高字节 */
	MODBUS_RE485_SEND();
	usart->tx_index = 0;	
	UART_Transmit_IT(usart);
	usart->rx_aframe = 0;	//清空和主机的通讯，避免通讯错误
	usart->rx_count = 0;		
}

/** 
  * 函数功能: 读N个输入寄存器(InputRegister)
  * 输入参数: _addr:从站地址,_reg:寄存器地址,_num:待读取的寄存器数量
  * 返 回 值: 无
  * 说    明: 填充数据发送缓存区,然后发送.
  */
void MB_ReadInputReg_04H(Usart_Type *usart,uint8_t _addr, uint16_t _reg, uint16_t _num)
{
  uint16_t crc = 0;
	
//	while(usart->tx_count!=0);
	wait_tx_count_reset(usart->tx_count);
	usart->tx_count = 0;	
	usart->txbuf[usart->tx_count++] = _addr;		    /* 从站地址 */
	usart->txbuf[usart->tx_count++] = 0x04;		      /* 功能码 */	
	usart->txbuf[usart->tx_count++] = _reg >> 8;	  /* 寄存器地址 高字节 */
	usart->txbuf[usart->tx_count++] = _reg;		      /* 寄存器地址 低字节 */
	usart->txbuf[usart->tx_count++] = _num >> 8;	  /* 线圈(bit)个数 高字节 */
	usart->txbuf[usart->tx_count++] = _num;		      /* 线圈(bit)个数 低字节 */

	crc = MB_CRC16(usart->txbuf,usart->tx_count);
  usart->txbuf[usart->tx_count++] = crc;	          /* crc 低字节 */
	usart->txbuf[usart->tx_count++] = crc>>8;		      /* crc 高字节 */
	MODBUS_RE485_SEND();
	usart->tx_index = 0;	
	UART_Transmit_IT(usart);
	usart->rx_aframe = 0;	//清空和主机的通讯，避免通讯错误
	usart->rx_count = 0;		
}
/** 
  * 函数功能: 写N个保持寄存器(HoldingRegister)
  * 输入参数: _addr:从站地址,_reg:寄存器地址,_num:待写入的寄存器数量,_databuf:待写入的寄存器数据
  * 返 回 值: 无
  * 说    明: 填充数据发送缓存区,然后发送._databuf的长度需 >= _num*2
  */
void MB_WriteNumHoldingReg_10H(Usart_Type *usart,uint8_t _addr, uint16_t _reg, uint16_t _num,uint8_t *_databuf)
{
  uint16_t i;
  uint16_t crc = 0;
	
//	while(usart->tx_count!=0);
	wait_tx_count_reset(usart->tx_count);
	usart->tx_count = 0;	
	usart->txbuf[usart->tx_count++] = _addr;		    /* 从站地址 */
	usart->txbuf[usart->tx_count++] = 0x10;		      /* 功能码 */	
	usart->txbuf[usart->tx_count++] = _reg >> 8;	  /* 寄存器地址 高字节 */
	usart->txbuf[usart->tx_count++] = _reg;		      /* 寄存器地址 低字节 */
	usart->txbuf[usart->tx_count++] = _num >> 8;	  /* 寄存器(16bits)个数 高字节 */
	usart->txbuf[usart->tx_count++] = _num;		      /*  低字节 */
  usart->txbuf[usart->tx_count++] = _num<<1;		  /* 数据个数 */

  for (i = 0; i < 2 * _num; i++)
  {
		usart->txbuf[usart->tx_count++]  = _databuf[i];		/* 后面的数据长度 */
	}

	crc = MB_CRC16(usart->txbuf,usart->tx_count);
  usart->txbuf[usart->tx_count++] = crc;	          /* crc 低字节 */
	usart->txbuf[usart->tx_count++] = crc>>8;		      /* crc 高字节 */
	MODBUS_RE485_SEND();
	usart->tx_index = 0;	
	UART_Transmit_IT(usart);
	usart->rx_aframe = 0;	//清空和主机的通讯，避免通讯错误
	usart->rx_count = 0;		
}

